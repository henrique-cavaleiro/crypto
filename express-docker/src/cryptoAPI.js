...
export async function fetchCryptoPrices(currency = "USD") {
    try {
        ...
        const dogeUrl = `https://api.coinbase.com/v2/prices/doge-${currency}/spot`;                 // examen
        const [btcData, ethData, dogeData] = await Promise.all([
            fetch(btcUrl).then(res => res.json()),
            fetch(ethUrl).then(res => res.json()),
            fetch(dogeUrl).then(res => res.json())          // examen
        ...
        return {
            ...
            DOGE: dogeData.data.amount          // examen

        };
    } catch (error) {
        console.error("Error fetching prices:", error);
        return { BTC: null, ETH: null, Doge: null };            // examen
    }
}
